<template>
  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="85px">
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" size="mini" v-hasPermi="['appTask:pagelist']" @click="handleQuery">搜索</el-button>
...
<!--      <el-col :span="1.5">
        ...
        <el-tooltip :disabled="!multiple" content="请先勾选需要删除的记录！" placement="top">
          <span>
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete"
              v-hasPermi="['appTask:deleteBatch']"
        ...
        <el-tooltip :disabled="!multiple" content="请先勾选需要导出的记录！" placement="top">
          <span>
            <el-button
              ...
              v-hasPermi="['appTask:exportExcelFile']"
    ...
    <el-table :height="tableMaxHeight" :cell-style="rowStyle" stripe border v-loading="loading" :data="tableDataList" @selection-change="handleSelectionChange" ref="objectTable">
      ...
      <el-table-column prop="taskContent" label="任务内容" :show-overflow-tooltip="true" width="600">
      </el-table-column>

      <el-table-column prop="taskStatus" label="任务状态" :show-overflow-tooltip="true" width="160">
        <template slot-scope="scope">
          <el-alert v-if="scope.row.taskStatus == '已办'" center :closable=false title="已办" type="success" effect="dark">
          </el-alert>
          <el-alert v-if="scope.row.taskStatus == '待办'" center :closable=false title="待办" type="error" effect="dark">
          </el-alert>
        </template>
      </el-table-column>
      ...
      <div class="bobo_dialog_div">
        ...
        <el-form ref="addFormRef" :model="addForm" :rules="rules" label-width="150px">
          ...
          <el-form-item label="任务内容" prop="taskContent">
            <el-input v-model="addForm.taskContent" placeholder="任务内容" clearable auto-complete="off" maxlength="50" show-word-limit style="width: 100%;"></el-input>
          </el-form-item>

          <el-form-item label="任务状态(待办;已办)" prop="taskStatus">
            <el-input v-model="addForm.taskStatus" placeholder="任务状态(待办;已办)" clearable auto-complete="off" maxlength="50" show-word-limit style="width: 100%;"></el-input>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        taskStatus: [{ required: true, message: '任务状态(待办;已办)不能为空', trigger: 'blur' }],
      },
    };
  },
  ...
  methods: {
    ...
    handleExportExcel() {
      ...
      this.download('appTask/exportExcelFile', {
        ...this.queryParams
    ...
    getList() {
      this.loading = true;
      getAppTaskTableData(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
      ...
      this.$refs["addFormRef"].validate(valid => {
          if (valid) {
            if (this.addForm.id != undefined) {
              updateAppTask(this.addForm).then(response => {
                this.open = false;
                this.getList();
              });
            } else {
              addAppTask(this.addForm).then(response => {
                this.open = false;
                this.getList();
              });
            }
