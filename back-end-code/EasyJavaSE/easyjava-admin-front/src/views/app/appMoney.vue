<template>
  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="85px">
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" size="mini" v-hasPermi="['appMoney:pagelist']" @click="handleQuery">搜索</el-button>
...
<!--      <el-col :span="1.5">
        ...
        <el-tooltip :disabled="!multiple" content="请先勾选需要删除的记录！" placement="top">
          <span>
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete"
              v-hasPermi="['appMoney:deleteBatch']"
        ...
        <el-tooltip :disabled="!multiple" content="请先勾选需要导出的记录！" placement="top">
          <span>
            <el-button
              ...
              v-hasPermi="['appMoney:exportExcelFile']"
    ...
    <el-table :height="tableMaxHeight" :cell-style="rowStyle" stripe border v-loading="loading" :data="tableDataList" @selection-change="handleSelectionChange" ref="objectTable">
      ...
      <el-table-column prop="moneyNum" label="记账金额" :show-overflow-tooltip="true" width="160">
      </el-table-column>

      <el-table-column prop="moneyStatus" label="账目类型" :show-overflow-tooltip="true" width="160">
        <template slot-scope="scope">
          <el-alert v-if="scope.row.moneyStatus == '收入'" center :closable=false title="收入" type="success" effect="dark">
          </el-alert>
          <el-alert v-if="scope.row.moneyStatus == '支出'" center :closable=false title="支出" type="error" effect="dark">
          </el-alert>
        </template>
      </el-table-column>

      <el-table-column prop="moneyDesc" label="记账描述" :show-overflow-tooltip="true" width="500">
      </el-table-column>
    ...
    >
      <div class="bobo_dialog_div">
        ...
        <el-form ref="addFormRef" :model="addForm" :rules="rules" label-width="150px">
          ...
          <el-form-item label="记账金额" prop="moneyNum">
            <el-input v-model="addForm.moneyNum" placeholder="记账金额" clearable auto-complete="off" maxlength="50" show-word-limit style="width: 100%;"></el-input>
          </el-form-item>

          <el-form-item label="账目类型(支出;收入)" prop="moneyStatus">
            <el-input v-model="addForm.moneyStatus" placeholder="账目类型(支出;收入)" clearable auto-complete="off" maxlength="50" show-word-limit style="width: 100%;"></el-input>
          </el-form-item>
...
import {
  getAppMoneyTableData,
  deleteAppMoney,
  addAppMoney,
  updateAppMoney,
  getAppMoneyList,
  getDataById,
} from "@/api/app/appMoney";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        moneyNum: [{ required: true, message: '记账金额不能为空', trigger: 'blur' }],
        moneyStatus: [{ required: true, message: '账目类型(支出;收入)不能为空', trigger: 'blur' }],
      },
    };
  },
  ...
  methods: {
    ...
    handleExportExcel() {
      ...
      this.download('appMoney/exportExcelFile', {
        ...this.queryParams
    ...
    getList() {
      this.loading = true;
      getAppMoneyTableData(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
      ...
      this.$refs["addFormRef"].validate(valid => {
          if (valid) {
            if (this.addForm.id != undefined) {
              updateAppMoney(this.addForm).then(response => {
                this.open = false;
                this.getList();
              });
            } else {
              addAppMoney(this.addForm).then(response => {
                this.open = false;
                this.getList();
              });
            }
