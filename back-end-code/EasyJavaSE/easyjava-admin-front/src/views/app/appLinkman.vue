<template>
  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="85px">
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" size="mini" v-hasPermi="['appLinkman:pagelist']" @click="handleQuery">搜索</el-button>
...
<!--      <el-col :span="1.5">
        ...
        <el-tooltip :disabled="!multiple" content="请先勾选需要删除的记录！" placement="top">
          <span>
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete"
              v-hasPermi="['appLinkman:deleteBatch']"
        ...
        <el-tooltip :disabled="!multiple" content="请先勾选需要导出的记录！" placement="top">
          <span>
            <el-button
              ...
              v-hasPermi="['appLinkman:exportExcelFile']"
    ...
    <el-table :height="tableMaxHeight" :cell-style="rowStyle" stripe border v-loading="loading" :data="tableDataList" @selection-change="handleSelectionChange" ref="objectTable">
      ...
      <el-table-column prop="manName" label="联系人名称" :show-overflow-tooltip="true" width="300">
      </el-table-column>

      <el-table-column prop="manPhone" label="联系人电话" :show-overflow-tooltip="true" width="200">
      </el-table-column>

      <el-table-column prop="manTop" label="是否置顶" :show-overflow-tooltip="true" width="160">
        <template slot-scope="scope">
          <el-alert v-if="scope.row.manTop == '不置顶'" center :closable=false title="不置顶" type="success" effect="dark">
          </el-alert>
          <el-alert v-if="scope.row.manTop == '置顶'" center :closable=false title="置顶" type="error" effect="dark">
          </el-alert>
        </template>
      </el-table-column>
      ...
      <div class="bobo_dialog_div">
        ...
        <el-form ref="addFormRef" :model="addForm" :rules="rules" label-width="150px">
          ...
          <el-form-item label="联系人名称" prop="manName">
            <el-input v-model="addForm.manName" placeholder="联系人名称" clearable auto-complete="off" maxlength="50" show-word-limit style="width: 100%;"></el-input>
          </el-form-item>

          <el-form-item label="联系人电话" prop="manPhone">
            <el-input v-model="addForm.manPhone" placeholder="联系人电话" clearable auto-complete="off" maxlength="50" show-word-limit style="width: 100%;"></el-input>
          </el-form-item>

          <el-form-item label="是否置顶(1:置顶;2:不置顶;)" prop="manTop">
            <el-input v-model="addForm.manTop" placeholder="是否置顶(1:置顶;2:不置顶;)" clearable auto-complete="off" maxlength="50" show-word-limit style="width: 100%;"></el-input>
          </el-form-item>
...
import {
  getAppLinkmanTableData,
  deleteAppLinkman,
  addAppLinkman,
  updateAppLinkman,
  getAppLinkmanList,
  getDataById,
} from "@/api/app/appLinkman";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        manName: [{ required: true, message: '联系人名称不能为空', trigger: 'blur' }],
        manPhone: [{ required: true, message: '联系人电话不能为空', trigger: 'blur' }],
        manTop: [{ required: true, message: '是否置顶(1:置顶;2:不置顶;)不能为空', trigger: 'blur' }],
      },
    };
  },
  ...
  methods: {
    ...
    handleExportExcel() {
      ...
      this.download('appLinkman/exportExcelFile', {
        ...this.queryParams
    ...
    getList() {
      this.loading = true;
      getAppLinkmanTableData(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
      ...
      this.$refs["addFormRef"].validate(valid => {
          if (valid) {
            if (this.addForm.id != undefined) {
              updateAppLinkman(this.addForm).then(response => {
                this.open = false;
                this.getList();
              });
            } else {
              addAppLinkman(this.addForm).then(response => {
                this.open = false;
                this.getList();
              });
            }
